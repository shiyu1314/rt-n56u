...
VOID PsRetrieveTimeout(RTMP_ADAPTER *pAd, STA_TR_ENTRY *tr_entry)
{
	...
	DBGPRINT(RT_DEBUG_TRACE, ("%s() Recover ps state(state %d to state %d) [wcid = %d]!!\n",
	...
	UCHAR wcid)
{
	...
	if (pfgForce)
	{
		...
		DBGPRINT(RT_DEBUG_TRACE | DBG_FUNC_PS, ("%s(%d): [wlan_idx=0x%x] PS Redirect mode(pfgForce = %d) is enabled. Send PC Clear command to FW.\n", 
